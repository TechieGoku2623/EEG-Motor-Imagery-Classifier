"""Load EDFs with MNE, bandpass, cut epochs, save .npz."""
...
def balance_rest(part: dict, rng: np.random.Generator) -> dict:
    """Keep fewer rest epochs so rest isn't ~50% of the set."""
    y = part["y"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--keep-all-rest",
        action="store_true",
        help="Keep every T0/rest epoch (rest will be about half the data)",
    )
