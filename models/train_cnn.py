"""Train EEGNet with the same splits as models/csp_baseline.py."""
...
def standardize_from_train(X_train: np.ndarray, X_test: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Z-score each channel using train mean/std."""
...
def train_one(
    ...
    # LazyLinear needs one forward pass before the optimizer
    with torch.no_grad():
...
def main() -> None:
    ...
    print(f"\nEEGNet  within={w:.3f}  LOSO={c:.3f}  gap={w-c:.3f}")
